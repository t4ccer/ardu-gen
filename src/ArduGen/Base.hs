...
{-# ANN module "HLint: Use typeRap" #-} -- Hlint detects typeOf in wrong context
...
instance ClassClass a => Type (Class a) where
  typeOf _ v = className (undefined :: a) <+> v
...
instance InhabitedType String where
...
instance ClassClass a => InhabitedType (Class a) where
...
externVar :: String -> Val lr a
externVar name = Val name
...
defineClass :: forall t nl names fields.
  ( ClassClass t
  , NameList names nl
  , StructBody nl fields
  , StructFields t nl fields
...
comment "" = emit "" --Do not render empty comments
comment s = emitLn $ "/*" <+> s <+> "*/"
...
toChar :: Fun (a -> IO Char)
toChar = fun $ trustMe "(char)"
